...
- **Webhook URL**
  ...
  - 將 nodered.com 換成你 Node-RED 的對外名稱，要有 SSL。
- **Allow bot to join group chats**
  - 允許加入聊天群組，可以回復群組信息或推送信息到群組。
  - 進去後在 Chat 的下方可以設定是否允許。

## Step 3

> **更新（2026）：**
>
> - 台灣銀行現在提供官方 JSON API：<https://rate.bot.com.tw/xrt/all/json>。建議把 http request 節點的 URL 改成這個端點，用 change/function 節點解析 JSON，就不需要脆弱的 CSS selector 跟寫死的陣列索引。
> - LINE Messaging API 的 push 訊息在多數方案下改為計費制（有免費額度），長期使用請留意用量。
...
回到 Line Developers 的頁面，在 Messaging API 的設定頁面下可以找到『Bot information』，下方有 QR Code 可以加 Line Bot 好友。
