...
> **更新（2026）：** UniFi Security Gateway（USG）已停產，本文的 `config.gateway.json` 做法僅適用於舊款 EdgeOS 系統的閘道器。現在的 UniFi OS 主機（UDM/UDR/UXG）請使用内建的 VPN Client 功能，或考慮 WireGuard／[Tailscale](https://tailscale.com/) 等方案。
...
如果直接在USG上設定OpenVPN跟路由的話會因為重新啟動而覆蓋掉原本的更動，所以要在 Unifi Controller 中加上 [config.gateway.json](https://help.ui.com/hc/en-us/articles/215458888) 檔案，內容如下：
...
依照你的Unifi網路設定更改你想要使用的 `<VLAN's Address/Subnet>` & `<VLANID>`，之後將 [config.gateway.json](https://help.ui.com/hc/en-us/articles/215458888) 複製到：`/unifi/data/sites/<unifi_base>/`
...
[USG Route VLAN over OpenVPN client][2]
 ...
 [1]: https://help.ui.com/hc/en-us/articles/215458888
