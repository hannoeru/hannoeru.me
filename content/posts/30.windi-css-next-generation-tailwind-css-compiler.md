...
> ⚠️ **注意（2026）：** Windi CSS は 2023 年にアーカイブされ、メンテナンスされていません。新規プロジェクトには [Tailwind CSS]（v3/v4）または [UnoCSS](https://unocss.dev/) を使用してください。本記事は履歴として残しています。
...
> 現在の構文は `npm create vite@latest` です（`npm init @vitejs/app` は廃止されました）。
