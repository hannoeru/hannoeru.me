...
> **更新（2026）：** VestaCP 已停止開發（最後一次更新為 2022 年）。建議改用持續維護的分支 [HestiaCP](https://hestiacp.com/)，它原生支援每個網域各自的 Let's Encrypt 憑證，不需要本文的手動設定。本文保留作為歷史參考。
...
到 Vesta 控制面板，點選右上角 Server -> Exim4 -> CONFIGURE 找到下列三行：
...
到 Vesta 控制面板，點選右上角 Server -> dovecot -> CONFIGURE 找到 `/etc/dovecot/conf.d/10-ssl.conf` 修改成：
...
```bash
#測試IMAP（記得加上 -servername 才會送出 SNI）
