...
> **更新（2026）：** 本文撰寫於 2019 年，EC2 控制台介面已改版，畫面與教學中的截圖不同，但流程相同。另外建議：
>
> - 系統映像檔請改用目前的 Ubuntu Server LTS（例如 24.04）。
> - PiVPN 現在預設推薦 WireGuard（設定更簡單、效能更好），安裝時選擇 WireGuard 即可，不需要輸入密碼的步驟。
> - 免費方案現在涵蓋多種機型（以點數計算），不限於 t2.micro。
...
7. 打開 Terminal 利用 scp 指令下載 OpenVPN 描述檔（.ovpn）到目前目錄：
...
> Windows 使用者可以使用 PuTTY 附帶的 `pscp` 或 [WinSCP](https://winscp.net/) 圖形化工具。
...
5. 享受沒有 IP 限制的世界。
6. 提示：可以為虛擬機綁定 Elastic IP，避免 IP 變更後需要重建 Profile。
