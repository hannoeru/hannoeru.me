...
[Demo](https://gh.hannoeru.me/js-aqi/)
...
> **更新（2026）：** 環保署已於 2023 年改制為環境部，上述舊端點已失效。新的空氣品質開放資料請改用環境部資料開放平臺：<https://data.moenv.gov.tw/>（例如 [AQI JSON API](https://data.moenv.gov.tw/dataset/detail/AQX_P_432)）。

但是因為我上傳到 Github Pages 會有混合內容的問題，所以還是掛了 Proxy。

Fetch API 照著官方文檔跟網路上找到的文章寫就沒問題了，Fetch API 有一個超棒的點是他可以直接用 response.ok
去判斷是否成功，其他參數設定也很簡單。
...
我翻譯了一篇寫得還不錯的文章：[XMLHttpRequest 與 Fetch API：2019 年最適合 Ajax 的是什麼？](/posts/xmlhttprequest-and-fetch-api-what-is-the-best-ajax-in-2019)
...
Google搜尋：Google Apps Script CORS Proxy（可用「Google Apps Script 做 CORS Proxy」找到許多中文／日文教學）
...
`[json()](https://developer.mozilla.org/zh-TW/docs/Web/API/Response/json)`
從響應對像中提取 JSON 。
