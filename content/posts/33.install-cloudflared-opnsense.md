...
> **Important:** Do **not** add these lines to `/etc/rc.conf`. OPNsense regenerates `/etc/rc.conf` from `config.xml` on boot and on every config apply, so hand-added lines are silently dropped. Use the per-service file `/etc/rc.conf.d/cloudflared` instead:

```bash
mkdir -p /etc/rc.conf.d
cat > /etc/rc.conf.d/cloudflared <<'EOF'
cloudflared_enable="YES"
cloudflared_mode="--no-autoupdate tunnel run --post-quantum"
EOF
```

> Note: this file may be reset by a firmware install/upgrade — re-create it afterwards if the service fails to start on boot.
...
> **Important:** Do **not** add this to `/etc/rc.conf` — OPNsense regenerates it from `config.xml` at boot and the line will be lost. Use the per-service file instead:
...
# For Method 1 - remotely-managed (token) tunnels can't be inspected
# from the CLI; check the status in the Cloudflare Zero Trust dashboard:
# https://one.dash.cloudflare.com/ -> Networks -> Tunnels

# For Method 2 - Test tunnel status (requires cert.pem from `tunnel login`)
...
cloudflared --no-autoupdate tunnel --token $(cat /usr/local/etc/cloudflared/token) run --post-quantum

# Check service logs
tail -f /var/log/cloudflared.log

# For token-based (remotely managed) tunnels, check status in the
# Cloudflare Zero Trust dashboard instead — the CLI cannot inspect them
```
