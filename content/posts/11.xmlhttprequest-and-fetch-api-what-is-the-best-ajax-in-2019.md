...
> **編者按（2026）：** 本文翻譯自 2019 年的 SitePoint 文章。以下部分結論已過時：Internet Explorer 已於 2022 年 6 月終止支援；Fetch 現已内建支援超時（`AbortSignal.timeout()`，2022 年起主流瀏覽器皆可用）與下載進度（串流 `response.body`）。「不支援超時」與「無法判斷進度」兩節的內容已不準確，僅保留作為歷史記錄。
...
以下 JavaScript 代碼使用 [XMLHttpRequest](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest) 示範了一個基本的 HTTP GET 請求到 [http://domain/service](http://domain/service)：
...
> **更新：** Fetch 現在可透過 `AbortSignal.timeout(ms)` 原生支援超時，不再需要以下包裝。

（以下為 2019 年當時的做法）Fetch 不支持超時，只要瀏覽器選擇，請求就會繼續。需要進一步的代碼來將 Fetch 包裝在另一個 Promise 中，例如
...
> **更新：** 現在可以透過串流讀取 `response.body`（ReadableStream）實作下載進度；上傳進度仍然沒有直接的事件支援。

（以下為 2019 年當時的情况）在撰寫本文時，Fetch 不支持進度事件。因此，無法報告文件上載或類似大型表單提交的狀態。

### XMLHttpRequest 與 Fetch API？ {#xmlhttprequestorfetch}

> **更新：** 以現在（2026）的觀點，除非有特殊需求，新專案一律使用 Fetch（或更高階的封裝如 Axios）。IE 已於 2022 年 6 月終止支援，以下關於 IE 相容性的段落僅供歷史參考。
...
翻譯自：<https://www.sitepoint.com/xmlhttprequest-vs-the-fetch-api-whats-best-for-ajax-in-2019/>
 ...
 [8]: https://adaptivepath.com/ideas/ajax-new-approach-web-applications/
 ...
 [16]: https://github.com/github/fetch
